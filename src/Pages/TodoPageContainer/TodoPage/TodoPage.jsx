...
function TodoPage ({Todo,TodoCreator,ChangeStatusCreator}) {
    ...
    function ModalForm() {
        ...
        function HandleSubmit(e) {
            ...
            TodoCreator(title, description, Todo.length + 1, priority, created, deadLine)
    ...
    QueuedTodo=Todo.filter(el=>el.status==='Queue')
    let [DevelopmentTodo,setDevelopmentTodo]=useState(Todo.filter(el=>el.status==='Development'))
    DevelopmentTodo=Todo.filter(el=>el.status==='Development')
    let [DoneTodo,setDoneTodo]=useState(Todo.filter(el=>el.status==='Done'))
    DoneTodo=Todo.filter(el=>el.status==='Done')
    ...
    function dropCardHandler(e, board) {
        ...
        ChangeStatusCreator(TodoId,board.text)
    }
    ActiveTodo=Todo.filter(el=>el.id===TodoId)
    return (<section className={c.TodoPage}>
        <div className={c.TodoPage__info}>
            ...
            <div className={c.info__dragDrop}>
                        {boards.map(board=><div className={c.Board} onDragOver={(e)=>dragOverHandler(e)} onDrop={(e)=>dropCardHandler(e,board)}>
                            <div className={c.Board__title}>{board.text}</div>
                            {board.todos.map(todo=>
                                <div className={`${c.item}`} draggable="true" onClick={(e)=>{console.log(e)}}
        ...
        {/*<div className={c.TodoPage__todos}>*/}
        {/*    {todo.map(el=>(<Todo value={el} />))}*/}
        {/*</div>*/}
